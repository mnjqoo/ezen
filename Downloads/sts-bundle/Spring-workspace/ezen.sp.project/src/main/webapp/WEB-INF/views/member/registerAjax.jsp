...
<html>
	...
	<body>
		...
		<div class="container">
			<form class="form-horizontal" method="post" action="${contextPath}/member/register">
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<button class="btn btn-warning idCheck" type="button" id="idCheck" onClick="fn_idCheck();" value="N">아이디 중복 확인</button>
					</div>
				</div>
				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-9">
						<input type="text" class="form-control msg" name="mgs" placeholder="알림" id="msg"/>
					</div>
					<div class="col-sm-offset-3 col-sm-9">
						<div id="msg2"></div>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="password" class="form-control" id="repwd" name="repwd" maxlength="20" placeholder="비밀번호 재입력"/>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-2">
						<input type="text" class="form-control" id="email" name="email" maxlength="100" placeholder="이메일 입력"/>
					</div>
				</div>
				<div class="form-group">
					<label for="register_yn" class="col-sm-3 control-label">회원가입 동의</label>
					<div class="col-sm-2">
						<label class="radio-inline">
							<input type="radio" id="register_yn" name="register_yn" value="Y" checked> 동의&nbsp&nbsp
						</label>
						<label class="radio-inline">
							<input type="radio" id="register_yn" name="register_yn" value="N"> 동의안함
						</label>
					</div>
				</div>
		...
		<script> 
		function fn_idCheck() { //아이디 중복 검사 버튼을 눌렀을 경우
			//alert($("#id").val());
			$.ajax({
				url: "/member/idCheck.do",
				type: "post", //method의 방식
				dataType: "json", //json방식: "변수, 값" 방식
				data: {"id" : $("#id").val()}, 
				success: function(data) { //data: 성공 했을 때 서버로부터 받는 데이터
					if(data == 1) { //아이디가 이미 존재하는 경우
						alert("이미 사용중인 아이디입니다.\n\n다른 아이디를 입력하십시오.");
					} else if(data == 0){ //존재하지 않는 경우
		...
		$(document).ready(function(){ 
			$(".cancel").on("click", function(){ //회원가입 취소 버튼을 눌렀을 경우
				location.href="/member/loginForm.do"; //로그인 화면으로 이동한다.
			});
			
			$("#submit").on("click", function(){ //id=submit 버튼을 눌렀을 경우
				if($("#id").val() == "") {
